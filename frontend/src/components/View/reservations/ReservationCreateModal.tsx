...
export function ReservationCreateModal({ date, onSuccess }: Props) {
  ...
  /* ===== マスタ系（基本コード） ===== */
  const { menuTypes, courses, allergies, guestTypes, status, purpose, cancel } =
  ...
  useEffect(() => {
    const loadTables = async () => {
      try {
        ...
        // enabled=true だけ出すならここで絞る
        setTables(data.filter((t: any) => t.enabled !== false));
      } catch (e) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* メニュー */}
      <Select value={menuTypeCode} onValueChange={setMenuTypeCode}>
        <SelectTrigger>
