...
const MY_PAGE_ENDPOINT = "/mypage/mypages" as const;
...
export function useMypage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<Mypage | null>(
    user ? `${MY_PAGE_ENDPOINT}/me` : null, // user が存在する時だけ API を叩く
    fetcher,
    {
  ...
  return {
    ...
    async create(payload: Mypage) {
      await apiClient.post(MY_PAGE_ENDPOINT, { mypage: payload });
      await mutate(); // 作成後にキャッシュを更新
    },

    // マイページ更新
    async update(id: number, payload: Partial<Mypage>) {
      await apiClient.patch(`${MY_PAGE_ENDPOINT}/${id}`, { mypage: payload });
